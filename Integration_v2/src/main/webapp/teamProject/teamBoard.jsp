...
<iframe src="../html/header.jsp" frameborder="0" scrolling="no"  style="margin:auto; width:100%; height:200px;" target="_top"></iframe>
			...
			<div id="contents">
				<div id="mypage">
					<h2><strong>자유게시판</strong><span></span></h2>
					...
					<c:forEach items="${list }" var="bdto">
								<tr>
									<td class="num">${bdto.boardid}</td>
									<td class="hit"><a href="#">${bdto.hit}</a></td>
									<td class="title"><a href="./teamBoardView.do?boardid=${bdto.boardid }">${bdto.title}</a></td>
									 <td class="writer">${bdto.memberid }</td>
									<td><fmt:formatDate value="${bdto.createdate }" pattern="yyyy-MM-dd" /></td>
								</tr>
							</c:forEach>
								<!--  반복 끝  -->
					<!-- 수정 -->
