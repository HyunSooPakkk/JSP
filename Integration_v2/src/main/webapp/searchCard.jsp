...
<script>
	$(function(){
		...
		$("header").click(function(e){
			...
			var mhtml = '<iframe src="./store_view.do?storeid='+e.currentTarget.children[0].value+'"  frameborder="0" scrolling="no" width="900px" height="1000px"></iframe>';
			
			
			console.log(mhtml);
			$("#modalviewChange").html(mhtml);
			console.log($("#modalviewChange").html());
		...
		$(".plusBtn>div").click(function(){
			
			location.href="./searchCard.do?sort=${sort}&start=${start}&end=${end+4}&query=${query}";
	...
	/*	
		...
		*/
		 ...
		 var height = $(".plusBtn").offset();
		 $('html, body').animate({scrollTop:height.top-1200},0);
...
<div id="store-search-wrap">
	<section id="search-tab-all" class="container-lnb" style="width:700px;">
		<form class="searchFrm" method="post" action="SearchQuery.do">
			<input type="hidden" name="viewtype" value="Card">
			<input type="hidden" name="sort" value="${sort}">
			
			<div class="search-group">
				<div class="search-bar-wrap">
					<input id="input-filter-query" type="text" name="query" placeholder="맛집 검색어 입력" class="form-control" value="${query}"> 
				</div> 
				<div class="search-btn-wrap"> 
					<span class="search-all">
						<button type="submit" data-toggle="tooltip" data-placement="left" title="" data-original-title="가게명 검색" class="btn btn-default">
		...
		<button type="button" class="filter-restaurant-btn-dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
			...
			<c:if test="${sort=='rate' }">
	      	...
	      	<c:if test="${sort=='nameAsc' }">
				<span class="label-btn">가나다순</span>
			</c:if>
			<c:if test="${sort=='createdDateDesc' }">
				<span class="label-btn">최신순</span>
			</c:if>
	      <span class="caret"></span>
	    </button>
	 <ul class="dropdown-menu">
      <li><a href="./searchCard.do?sort=rate&query=${query}" data-value="rate">평점순</a></li>
      <li><a href="./searchCard.do?sort=nameAsc&query=${query}" data-value="nameAsc">가나다순</a></li>
      <li><a href="./searchCard.do?sort=createdDateDesc&query=${query}" data-value="createdDateDesc">최신순</a></li>
    </ul>
	</div>
</div>
</div>
...
<c:forEach begin="${start-1 }" end="${end-1 }" step="2" varStatus="status" >
...
<li class="restaurant-thumb-item" data-id="29206" style="position: absolute; left: 0px; top: 0px;">
    ...
    <div class="thumb-restaurant br-card" data-href="/restaurants/29206" style="cursor: pointer">
        ...
        <header>
			<input type="hidden" class="storeId" value="${list[status.index].storeId}">
          <div class="thumb-restaurant-img embed-responsive embed-responsive-16by9">
            ...
            <div class="header-status">
                <ol class="foodtype">
                	<c:set var="text" value="${fn:split(list[status.index].keywordId,',')}" />
                	<c:forEach var="key" items="${text}">
                		<li>${key}</li>
                	</c:forEach>  	
                   <!--  <li>일식</li>
                    <li>일식오마카세</li>
            ...
            <div class="header-title">
              ...
              <div class="starfix">
                ...
                <c:forEach begin="1" end="${list[status.index].rate/list[status.index].ratecounter}">
	                 <li>
				...
				<p>${list[status.index].rate/list[status.index].ratecounter}</p>
              </div>

              <div class="clearfix">
                    <h3>
                       ${list[status.index].storeName}
                    </h3>
        ...
        <div class="thumb-caption">
            <div class="info">
                <dl class="dl-horizontal">
                    ...
                    <dd><a href="tel:${list[status.index].phone}"> ${list[status.index].phone} </a></dd>
                    <dt><i class="icon-fa-location"></i>주소</dt>
                    <dd> ${list[status.index].address } <button type="button" class="copy-address" data-toggle="popover" data-placement="bottom" data-content="복사 되었습니다"><i class="icon-fa-copy"></i></button></dd>
                </dl>
            </div>
            <div class="content">
                ${list[status.index].storeContent }
            </div>
...
<li class="restaurant-thumb-item" data-id="28946" style="position: absolute; left: 336px; top : 0px;">
    ...
    <div class="thumb-restaurant br-card" data-href="/restaurants/28946" style="cursor: pointer">
        ...
        <header>
       		<input type="hidden" class="storeId" value="${list[status.index+1].storeId}">
          <div class="thumb-restaurant-img embed-responsive embed-responsive-16by9">
            <div class="embed-responsive-item bg-cover" style="background-image:url(./images/tempfoodImage.jpg);"></div>
          </div>
            ...
            <div class="header-title">
              ...
              <div class="starfix">
                <ul class="stars pull-left">
                  <c:forEach begin="1" end="${list[status.index+1].rate/list[status.index+1].ratecounter}">
	                 <li>
	                    <img class="img-star" src="./images/star_yellow.png">
	                 </li>
                 </c:forEach>
                </ul>
                <p>${list[status.index+1].rate/list[status.index+1].ratecounter}</p>
              </div>

              <div class="clearfix">
                    <h3>
                        ${list[status.index+1].storeName}
                    </h3>
        ...
        <div class="thumb-caption">
            <div class="info">
                <dl class="dl-horizontal">
                    <dt><i class="icon-fa-phone"></i>전화</dt>
                    <dd><a href="tel:02-591-2672">${list[status.index+1].phone}</a></dd>
                    <dt><i class="icon-fa-location"></i>주소</dt>
                    <dd>${list[status.index+1].address} <button type="button" class="copy-address" data-toggle="popover" data-placement="bottom" data-content="복사 되었습니다"><i class="icon-fa-copy"></i></button></dd>
                </dl>
            </div>
            <div class="content">
                ${list[status.index+1].storeContent}
            </div>
            <div class="evaluate-btn-wrap">
              <a class="anchor-restaurant-arrow" href="#">평가하기<i class="icon-fa-angle-right"></i></a>
            </div>
...
<!-- 모 달 모 달 모 달 모 달 모 달 모 달 모 달 모 달 모 달 모 달 모 달  -->
<div class="modal view">
        <div class="modal_body" id="modalviewChange">
           <iframe src="./store_view.jsp"  frameborder='0' scrolling="no" width="900px" height="1000px"></iframe>
        </div>
 </div>

<div class="modal review">
        <div class="modal_body review">
           <iframe src="./review/review_write.jsp"  frameborder='0' scrolling="yes" width="900px" height="1000px"></iframe>
        </div>
 </div>
