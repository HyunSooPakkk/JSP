	...
	public ArrayList<BoardDto> bList(String bCategory, String bSearch, int startRow, int endRow) {
		try {
			...
			if(bCategory==null) {
				...
				query = "select * from (select row_number() over(order by bgroup desc, bstep asc) rnum, a.* from board a where btitle like '%'||?||'%' or bcont like '%'||?||'%') where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				...
				query = "select * from (select row_number() over(order by bgroup desc, bstep asc) rnum, a.* from board a where bcont like '%'||?||'%') where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, bSearch);
				pstmt.setInt(2, startRow);
				pstmt.setInt(3, endRow);
	...
	public int listCount(String bCategory, String bSearch) {
		try {
			...
			if(bCategory==null) {
				...
				query = "select count(*) listCount from board where btitle like '%'||?||'%' or bcont like '%'||?||'%'";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, bSearch);
				pstmt.setString(2, bSearch);
				...
				query = "select count(*) listCount from board where bcont like '%'||?||'%'";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, bSearch);
