...
public class BoardDao {
	...
	public ArrayList<BoardDto> n_listSelect(String category, String sword, int startRow, int endRow) {
		try {
			conn = getConnection();
			if(category==null) {
				query = "select * from (select row_number() over (order by bgroup desc,bstep asc) rnum, a.* from board2 a) where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				pstmt.setInt(1, startRow);
				pstmt.setInt(2, endRow);
			}else if(category.equals("all")) {
				query = "select * from (select row_number() over (order by bgroup desc,bstep asc) rnum, a.* from board2 a where btitle like '%'||?||'%' or bcontent like '%'||?||'%') where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				...
				query = "select * from (select row_number() over (order by bgroup desc,bstep asc) rnum, a.* from board2 a where btitle like '%'||?||'%') where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, sword);
				pstmt.setInt(2, startRow);
				pstmt.setInt(3, endRow);
			}else {
				query = "select * from (select row_number() over (order by bgroup desc,bstep asc) rnum, a.* from board2 a where bcontent like '%'||?||'%') where rnum between ? and ?";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, sword);
				pstmt.setInt(2, startRow);
				pstmt.setInt(3, endRow);
	...
	public int nListCount(String category, String sword) {
		try {
			conn = getConnection();
			if(category==null) { //
				query = "select count(*) listCount from board2";
				pstmt = conn.prepareStatement(query);
				rs = pstmt.executeQuery();
				if(rs.next()) {
				...
				query = "select count(*) listCount from board2 where btitle like '%'||?||'%' or bcontent like '%'||?||'%' ";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, sword);
				pstmt.setString(2, sword);
				rs = pstmt.executeQuery();
				...
				query = "select count(*) listCount from board2 where btitle like '%'||?||'%'";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, sword);
				rs = pstmt.executeQuery();
				if(rs.next()) {
				...
				query = "select count(*) listCount from board2 where bcontent like '%'||?||'%'";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, sword);
				rs = pstmt.executeQuery();
				if(rs.next()) {
	...
	public BoardDto selectOne(int bno2) {
		try {
			conn = getConnection();
			query = "select * from board2 where bno=?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, bno2);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public int insert(BoardDto bdto2) {
		try {
			conn = getConnection();
			query = "insert into board2 values(board_seq.nextval,?,?,sysdate,?,board_seq.currval,0,0,1,?)";
			pstmt = conn.prepareStatement(query);
			//1,2,
	...
	public void stepUp(int bgroup2, int bstep2) {
		try {
			conn = getConnection();
			query = "update board2 set bstep=bstep+1 where bgroup=? and bstep>?";
			pstmt = conn.prepareStatement(query);
			//1,2,
	...
	public int replyInsert(BoardDto bdto2) {
		try {
			conn = getConnection();
			query = "insert into board2 values(board_seq.nextval,?,?,sysdate,?,?,?,?,1,?)";
			pstmt = conn.prepareStatement(query);
			//1,2
			pstmt.setString(1, bdto2.getBtitle());
			pstmt.setString(2, bdto2.getBcontent());
	...
	public int update(BoardDto bdto2) {
		try {
			conn = getConnection();
			query = "update board2 set btitle=?,bcontent=?,bfile=? where bno=?";
			pstmt = conn.prepareStatement(query);
			//1,2
			pstmt.setString(1, bdto2.getBtitle());
			pstmt.setString(2, bdto2.getBcontent());
	...
	public void bhitUp(int bno2) {
		try {
			conn = getConnection();
			query = "update board2 set bhit = bhit+1 where bno=?";
			pstmt = conn.prepareStatement(query);
			//1,2
			pstmt.setInt(1, bno2);
			pstmt.executeUpdate();
